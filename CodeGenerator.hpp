...
struct CGWarning{
  std::string msg;
  FileLocation pos;
  CGWarning(const std::string& argMsg,FileLocation argPos):msg(argMsg),pos(argPos){}
};
...
public:
  ...
  struct OpPair{
    ...
    bool skipNext;
    mutable bool owned;
    OpPair(FileLocation argPos,ZorroVM* argVm,OpBase* argFirst=0):vm(argVm),first(new ZCode(vm,argFirst)),last(argFirst),link(0),pos(argPos),skipNext(false),owned(true)
    {
      if(*first)
      {
        first->pos=pos;
    ...
    OpPair(FileLocation argPos,ZorroVM* argVm,OpBase* argFirst,OpBase* argLast):vm(argVm),first(new ZCode(vm,argFirst)),last(argLast),link(0),pos(argPos),skipNext(false),owned(true){}
    //merge two pairs
    ...
    void fixLast(OpBase* next)
    {
      ...
      for(FixVector::iterator it=efixes.begin(),end=efixes.end();it!=end;++it)
      {
        **it=next;
      }
      efixes.clear();
  ...
  */
  ...
  struct ExprContext{
    ...
    JVector jumps;
    OpBase* lastBinOp;
    ExprContext(SymbolsInfo* argSi,const OpArg& argDst=OpArg()):si(argSi),dst(argDst),lvalue(false),constAccess(false),ifContext(false),lastBinOp(0){}
