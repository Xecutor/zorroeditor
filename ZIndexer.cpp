...
static std::string getFullSymName(SymInfo* sym)
{
    ...
    if(sym->getParent() && sym->getParent()->st == sytGlobalScope)
    {
        return rv;
    }
    while(sym->getParent())
...
void ZIndexer::indexate(FileReader* fr, bool initial)
{
    ...
    if(res)
    {
        it->second->lastParseResult.values.clear();
        it->second->lastParseResult.values.splice(it->second->lastParseResult.values.end(), res->values);
    } else if(parseOk)
    {
        it->second->lastParseResult.values.clear();
    }
...
static Expr* findNearestSubExpr(Expr* e, int line, int col)
{
    ...
    if(e->lst)
    {
        for(auto it = e->lst->values.rbegin(), end = e->lst->values.rend(); it != end; ++it)
        {
            Expr* ee = it->get();
            if(ee && (rv = findNearestSubExpr(ee, line, col)))
            {
                return rv;
            }
        }
    }
    if(e->e3 && isInside(line, col, e->e3) && (rv = findNearestSubExpr(e->e3, line, col)))
    {
        return rv;
    }
    if(e->e2 && isInside(line, col, e->e2) && (rv = findNearestSubExpr(e->e2, line, col)))
    {
        return rv;
    }
    if(e->e1 && isInside(line, col, e->e1) && (rv = findNearestSubExpr(e->e1, line, col)))
    {
        return rv;
    }
    return e;
...
void
ZIndexer::getCompletions(const std::string& fn, bool ns, ScopeSym* scope, int line, int col, const std::string& start,
...
void ZIndexer::getCompletions(StmtList* stmtLst, bool ns, int line, int col, const std::string& start,
                              std::list<SymInfo*>& lst)
{
    if(!stmtLst)
    {
    ...
    Statement* prev = 0;
    bool found = false;
    for(auto& st:stmtLst->values)
    {
        if(isInside(line, col, st.get()))
        {
            prev = st.get();
            found = true;
            break;
        }
        if(prev && isLess(line, col, st.get()))
        {
            found = true;
            break;
        }
    ...
    if(prev)
    {
        ...
        if(!ex)
        {
            for(auto s:subStmt)
            {
                if(s && !s->values.empty() && isInside(line, col, s->values.front()->pos, s->values.back()->end))
                {
    ...
    {
        ex = prev->as<ExprStatement>().expr.get();
    }
    if(ex)
    {
    ...
    switch(scope->st)
    {
        ...
        case sytClass:
        {
            ...
            if(ci->def)
            {
                ...
                if(cds.parent && cds.parent->args &&
                   isInside(line, col, cds.parent->args->values.front()->pos, cds.parent->args->values.back()->end))
                {
                    for(auto& ex:cds.parent->args->values)
                    {
                        SymInfo* rv = findSymbolAt(ex.get(), line, col);
                        if(rv)
                        {
                            return rv;
                        }
...
SymInfo* ZIndexer::findSymbolAt(StmtList* stl, int line, int col)
{
    ...
    for(auto& stptr:stl->values)
    {
        if(!isInside(line, col, stptr.get()))
        {
            continue;
        }
        Statement& st = *stptr;
...
SymInfo* ZIndexer::findSymbolAt(Expr* ex, int line, int col)
{
    ...
    } else
    {
        ...
        } else
        {
            ...
            if(ex->lst)
            {
                for(auto& e:ex->lst->values)
                {
                    if((rv = findSymbolAt(e.get(), line, col)))
                    {
                        return rv;
                    }
                }
            ...
            switch(st.st)
            {
                ...
                case stClass:
                {
                    ...
                    if(cds.parent)
                    {
                        ...
                        {
                            auto* ci = (ClassInfo*) pc;
                            if(!ci->parentClass)
                            {
                                StmtList tmp;
                                tmp.values.emplace_back(ci->def);
                                try
                                {
                                    indexate(&tmp);
                                }
                    ...
                    if(fillSymList)
                    {
                        sl.push_back(SymbolRef(cds.name, ci));
                    }
                ...
                case stClassMemberDef:
                {
                    ...
                    if(fillSymList)
                    {
                        sl.push_back(SymbolRef(m.name, si));
                    }
                }
                    break;
                case stProp:
                {
                    ...
                    } else if(pst.getName.val.get())
                    {
                        ...
                        if(fillSymList)
                        {
                            sl.push_back(SymbolRef(pst.getName, si));
                        }
                    }
                    if(pst.setFunc)
                    {
                        ...
                        m->def = pst.setFunc;
                        ps->setMethod = m;
                        int idx = cg.si->registerArg(*pst.setFunc->args->values.front());
                        SymInfo* ai = m->locals[idx];
                        if(fillSymList)
                        {
                            sl.push_back(SymbolRef(*pst.setFunc->args->values.front(), ai));
                        }
                    ...
                    } else if(pst.setName.val.get())
                    {
                        ...
                        if(fillSymList)
                        {
                            sl.push_back(SymbolRef(pst.setName, si));
                        }
                    }
                }
                    break;
...
void ZIndexer::indexateFunc(FuncDeclStatement& fds)
{
    ...
    if(isMethod)
    {
        ...
        if(fillSymList)
        {
            sl.push_back(SymbolRef(name, si));
        }
    } else
    {
    ...
    if(fds.args)
    {
        for(auto& it : fds.args->values)
        {
            ...
            if(fillSymList)
            {
                sl.push_back(SymbolRef(fp, sym));
            }
            indexateExpr(fp.defValue.get());
        }
    }
    if(isMethod)
    {
    ...
    switch(expr->et)
    {
        case etAssign:
        {
            if(expr->e1->et == etVar)
            {
                ...
                if(fillSymList)
                {
                    sl.push_back(SymbolRef(expr->e1->getSymbol().name, si));
                }
            } else
            {
        ...
        case etVar:
        {
            ...
            if(fillSymList)
            {
                sl.push_back(SymbolRef(expr->getSymbol().name, si));
            }
        }
            break;
        case etNumArg:
        ...
        case etSeqOps:
        {
            ...
            if(fillSymList)
            {
                sl.push_back(SymbolRef(symName.name, si));
            }
        }
            /* no break */
        default:
    ...
    for(auto& it : sl->values)
    {
        ...
        switch(st.st)
        {
            ...
            case stEnum:
            {
                ...
                if(est.items)
                {
                    for(auto& v:est.items->values)
                    {
                        if(v->et == etVar)
                        {
                            cg.si->registerGlobalSymbol(v->getSymbol().name,
                                                        new SymInfo(v->getSymbol().name, sytGlobalVar));
                        } else if(v->et == etAssign && v->e1->et == etVar)
                        {
                            cg.si->registerGlobalSymbol(v->e1->getSymbol().name,
                                                        new SymInfo(v->e1->getSymbol().name, sytGlobalVar));
                        }
                    }
                }
            }
            ...
            case stExpr:
            {
                ...
                {
                    cg.si->registerGlobalSymbol(est.expr->e1->getSymbol().name,
                                                new SymInfo(est.expr->e1->getSymbol().name, sytGlobalVar));
                }
            }
                break;
            default:
...
{
    if(!stmtList || stmtList->values.empty())
    {
        return 0;
    }
    std::vector<decltype(stmtList->values.begin())> begins, ends;
    begins.push_back(stmtList->values.begin());
    ends.push_back(stmtList->values.end());
    int rv = 0;
    std::vector<StmtList*> subStmt;
        ...
        for(; it != end; ++it)
        {
            ...
            if(isInsideEx(line, col, &st))
            {
                switch(st.st)
                {
                    ...
                    case stExpr:
                    {
                        return rv + getExprDepth(line, col, st.as<ExprStatement>().expr.get());
                    }
                        break;
                    case stProp:
                    {
                        ...
                        if(ps.getFunc && isInside(line, col, ps.getFunc->pos, ps.getFunc->end))
                        {
                            ...
                            if(ps.getFunc->body)
                            {
                                begins.push_back(ps.getFunc->body->values.begin());
                                ends.push_back(ps.getFunc->body->values.end());
                            }
                        }
                        if(ps.setFunc && isInside(line, col, ps.setFunc->pos, ps.setFunc->end))
                        {
                            ...
                            if(ps.setFunc->body)
                            {
                                begins.push_back(ps.setFunc->body->values.begin());
                                ends.push_back(ps.setFunc->body->values.end());
                            }
                        }
                    }
                        break;
                    case stIf:
                    {
                        ...
                        if(ifst.thenList && ifst.thenList->values.front()->pos < ifst.cond->pos)
                        {
                            return rv;
                        }
                        if(ifst.thenList)
                        {
                            if(isInside(line, col, ifst.thenList->values.front()->pos, ifst.thenList->values.back()->end))
                            {
                                begins.push_back(ifst.thenList->values.begin());
                                ends.push_back(ifst.thenList->values.end());
                                break;
                            }
                        }
                        if(ifst.elseList)
                        {
                            if(isInside(line, col, ifst.elseList->values.front()->pos, ifst.elseList->values.back()->end))
                            {
                                begins.push_back(ifst.elseList->values.begin());
                                ends.push_back(ifst.elseList->values.end());
                                break;
                            }
                        }
                        if(ifst.elsifList)
                        {
                            for(auto& s:ifst.elsifList->values)
                            {
                                if(isInside(line, col, s->pos, s->end))
                                {
                                    auto& elif = s->as<IfStatement>();
                                    if(elif.thenList)
                                    {
                                        begins.push_back(elif.thenList->values.begin());
                                        ends.push_back(elif.thenList->values.end());
                                    }
                                    break;
                                }
                            }
                    ...
                    case stForLoop:
                    {
                        if(st.st == stWhile)
                        {
                            ...
                            if(wst.body && wst.body->values.front()->pos < wst.cond->pos)
                            {
                                return rv;
                            }
                        } else
                        {
                            ...
                            if(fst.body && fst.body->values.front()->pos < fst.expr->pos)
                            {
                                return rv;
                            }
                        }
                    ...
                    case stLiter:
                    {
                        ...
                        for(auto s:subStmt)
                        {
                            if(!s->values.empty())
                            {
                                if(isInside(line, col, s->values.front()->pos, s->values.back()->end))
                                {
                                    begins.push_back(s->values.begin());
                                    ends.push_back(s->values.end());
                                    break;
                                }
                            }
                        }
...
int ZIndexer::getExprDepth(int line, int col, Expr* expr)
{
    switch(expr->et)
    {
        ...
        case etCall:
        {
            if(expr->lst)
            {
                for(auto& e:expr->lst->values)
                {
                    if(isInside(line, col, e.get()))
                    {
                        return 1 + getExprDepth(line, col, e.get());
                    }
                }
            }
            return 1;
        ...
        case etFunc:
        {
            ...
            break;
        default:
            if(expr->e1 && isInside(line, col, expr->e1))
            {
                return getExprDepth(line, col, expr->e1);
            }
            if(expr->e2 && isInside(line, col, expr->e2))
            {
                return getExprDepth(line, col, expr->e2);
            }
            if(expr->e3 && isInside(line, col, expr->e3))
            {
                return getExprDepth(line, col, expr->e3);
            }
            break;
    }
    return 0;
