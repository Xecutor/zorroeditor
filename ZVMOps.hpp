...
struct OpBinOp:OpDstBase{
  ...
  void getArgs(ArgsVector& args)
  {
    if(ot!=otAssign)args.push_back(&left);
...
struct OpTerOp:OpBinOp{
  OpArg arg;
  OpTerOp(const OpArg& argLeft,const OpArg& argArg,const OpArg& argRight,const OpArg& argDst):OpBinOp(argLeft,argRight,argDst),
          arg(argArg)
  {}
  virtual ~OpTerOp(){}
  void getArgs(ArgsVector& args)
...
struct OpMakeRange:OpBase{
  ...
  bool inclusive;
  OpMakeRange(const OpArg& argStart,const OpArg& argEnd,const OpArg& argStep,const OpArg& argDst,bool argInclusive);
  virtual ~OpMakeRange(){}
...
struct OpJumpIfBinOp:OpJumpBase{
  OpArg left,right;
  OpJumpIfBinOp(const OpArg& argLeft,const OpArg& argRight,OpBase* argElseOp=0):OpJumpBase(argElseOp),left(argLeft),right(argRight){}
  void getArgs(ArgsVector& args)
  {
    args.push_back(&left);
    args.push_back(&right);
...
struct OpCallMethod:OpDstBase{
  ...
  index_type args;
  bool namedArgs;
  OpCallMethod(index_type argArgs,const OpArg& argSelf,index_type argMethodIdx,const OpArg& argResult,bool argNamedArgs=false);
...
struct OpFormat:OpDstBase{
  ...
  int w,p;
  OpFormat(int argW,int argP,OpArg argSrc,OpArg argDst,OpArg argWidth,OpArg argPrec,OpArg argFlags,OpArg argExtra);
  void getArgs(ArgsVector& args)
  {
    args.push_back(&src);
    args.push_back(&dst);
