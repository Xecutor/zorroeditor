...
void Image::load(const char* argFileName)
{
  ...
  if(image->format->BytesPerPixel<=3 || !isPowerOf2(image->w) || !isPowerOf2(image->h))
  {
    ...
    texRect.size=Pos(1.0*image->w/w,1.0*image->h/h);
...
Color Image::getPixel(int x,int y)
{
  ...
  Color rv;
  rv.r=1.0*((pix&s.format->Rmask)>>s.format->Rshift)/255.0;
  rv.g=1.0*((pix&s.format->Gmask)>>s.format->Gshift)/255.0;
  rv.b=1.0*((pix&s.format->Bmask)>>s.format->Bshift)/255.0;
  if(s.format->Amask)
  {
    rv.a=1.0*((pix&s.format->Amask)>>s.format->Ashift)/255.0;
  }else
  {
    rv.a=1.0;
  }
...
void Image::setPixel(int x,int y,Color clr)
{
  ...
  px+=x*s.format->BytesPerPixel;
  uint32_t& pix=*((uint32_t*)px);
  unsigned char r=clr.r*255;
  unsigned char g=clr.g*255;
  unsigned char b=clr.b*255;
  unsigned char a=clr.a*255;
